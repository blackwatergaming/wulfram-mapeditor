...
Each map occupies `maps/<slug>/` and contains exactly five UTF-8, LF-terminated
text files:
...
writer. Numeric values retain the precision accepted by the original writers.
...
new-server `base-layout.json`, and editor backup files are deterministic release
